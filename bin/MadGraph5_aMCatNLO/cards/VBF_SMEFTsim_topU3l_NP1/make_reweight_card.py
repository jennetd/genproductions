...
def getWeightName( reweight ):
    name = "_".join( [ ("%s_%8.6f"%( reweight[2*i], reweight[2*i+1] )).rstrip('0').rstrip('.') for i in range(len(reweight)/2) ] ) 
...
def make_reweight_card( filename, reweights, referencepoint, order_dict ):
    ...
    with open(filename, "w") as out_file:
        ...
        for index, reweight in enumerate(reweights):
            name = 'EFTrwgt' + str(index) + '_' + getWeightName( reweight )
